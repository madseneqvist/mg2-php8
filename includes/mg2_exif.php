...
function tagid2name($id) {
	switch ($id) {
		...
		case 0xa001: return "ColorSpace"; break;			// kh_mod 0.2.0, add
		...
		case 0x013e: return "WhitePoint"; break;			// kh_mod 0.2.0, changed
		...
		case 0x9205: return "MaxApertureWidth"; break;	// kh_mod 0.1.0, changed
...
function dealwithtag($tag, $format, $data, $length, $align) {
	...
	switch ($format_type[$format]) {
		...
		case "URATIONAL":
			...
			$w = true; // echo '<br />"$tag, URATIONAL": ',dechex($tag),' | ',$val;
			break;
		case "SRATIONAL":
			$val = getrational(substr($data, 0, 8), $align, $tag, "S");	// kh_mod 0.1.0, changed
			$w = true; // echo '<br />"$tag, SRATIONAL": ',dechex($tag),' | ',$val;
			break;
		case "UNDEFINED":
			switch ($tag) {
				case 0xa300:
